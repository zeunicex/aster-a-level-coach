...
export function nextReviewDate({ correct, confidence = "Medium", usedHint = false, evidence = 0, today = dateKey() }) {
  ...
  if (confidence === "Medium") return addReviewDays(today, 3);
...
export function objectiveNeedsPractice(objective, today = dateKey()) {
  if (typeof objective.mastered === "boolean") return !objective.mastered || isReviewDue(objective.due, today);
...
export function reliableMastery(objective, attempts, today = dateKey()) {
  ...
  const independent = recent.filter((attempt) => attempt.correct && !attempt.usedHint && attempt.confidence !== "Low");
  ...
  return independent.length >= 3 && formats.size >= 2 && [...formats].some((format) => format !== "mcq") && accuracy >= 0.75;
}
